...
function documentGetElementByIdOrError<T>(id:string)  {
...
const canva = documentGetElementByIdOrError<HTMLCanvasElement>("game"); 
canva.width = WIDTH;
canva.height = HEIGHT;
canva.style.background = 'white';
const ctx = getCanvasCTX("game");
function convertFPSToMiliseconds(fps:number) :number {
 ...
 function getCanvasCTX(id:string): CanvasRenderingContext2D {

	const canvas = documentGetElementByIdOrError<HTMLCanvasElement>(id); 
...
function calculateBlocksPosition(direccion:Directions, number_of_blocks:number){

	const n = [];
	let result = 0;
	let position_to_start_calculating :number = 0; 
	let blocks_per_to_calculate:number = 0;
	if(direccion === 'y') {
...
function renderBlock(x_position:number, y_position:number, w: number, h:number): void{
	ctx.beginPath();
...
function renderUpperBlocks(): void {
...
function renderGame(){
  ...
  window.addEventListener("keypress", (e) => {
	...
	//MOVE LEFT
	if(e.key === 'h') current_player_position = current_player_position - SPEED_MOVE; 
	
	//MOVE RIGHT 
	if(e.key === 'l') current_player_position = current_player_position + SPEED_MOVE; 
	
	// RESET POSITION
	if(e.key === 'r') current_player_position = PLAYER_STARTING_POINT
}) 

renderGame();
const stop_game = documentGetElementByIdOrError<HTMLButtonElement>("stop"); 
